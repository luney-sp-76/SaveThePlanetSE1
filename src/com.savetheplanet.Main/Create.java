...
final class Create {
    ...
    /**
     * @returns reads fields and squares, List of Squares as Board.
     * Jaszon
     */
     ...
     * Jaszon
     */
    private static Player validateName(Scanner scan, String str, List<Player> players, int i) {
    ...
    /**
     * @param scan Scanner
     * @param str  Input number of players
    ...
    private static int playerCount(Scanner scan, String str) {
        while (true) {
            try {
                ...
                if (input < 2 || input > 4) {
                    throw new IllegalArgumentException("That number was not in range.");
                } else {
                    return input;
                }
            } catch (NumberFormatException e) {
                System.err.println(e.getMessage().replaceFirst(".*", "That was not a number."));
